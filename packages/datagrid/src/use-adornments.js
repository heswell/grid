...
const useAdornments = ({children}) => {
  const [adornments, setAdornments] = useState(children ? getAdornments(children) : NO_ADORNMENTS)
  return adornments;
}
...
function getAdornment(child){
  if (child.type === Header || child.type === Footer || child.type === InlineHeader){
    const type = child.type === Header ? 'header' : child.type === Footer ? 'footer' : 'inlineHeader';
