...
export const buildContextMenuDescriptors = gridModel => (location, options) => {
  ...
  const descriptors = [];
    ...
    if (location === 'header') {
      ...
      descriptors.push({label: 'Hide Column', action: Action.ColumnHide, options})
  }

  if (gridModel.visualLinks){
    gridModel.visualLinks.forEach(linkDescriptor => {
      descriptors.push({label: `Link to ${linkDescriptor.link.toTable}`, action: Action.LinkTable, options: linkDescriptor})
    })
  }
