...
export const isLastIndex2 = (ctx) => 
    ctx.idx2 === ctx.model.fields[ctx.idx].type.length - 1;
...
export const transitionNext = (cond= alwaysTrue) => [
    ...
    { target: '#focus-composite', actions: setNextField, cond: (c,e) => cond(c, e) && isComposite(c.nextField(e))},
    { target: '#focus-text-input', actions: setNextField, cond: (c,e) => cond(c, e) && isTextInput(c.nextField(e))},
    { target: '#focus-select', actions: setNextField, cond: (c,e) => cond(c, e) && isSelect(c.nextField(e))},
    { target: '#focus-combo', actions: setNextField, cond: (c,e) => cond(c, e) && isCombo(c.nextField(e))},
    { target: '#edit-toggle', actions: setNextField, cond: (c,e) => cond(c, e) && isToggle(c.nextField(e))}
];
